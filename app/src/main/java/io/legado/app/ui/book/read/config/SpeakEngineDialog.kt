    ...
    override fun onMenuItemClick(item: MenuItem?): Boolean {
        when (item?.itemId) {
            ...
            R.id.menu_add_advanced -> {
                try {
                    // 1. 使用 Dialog 弹窗，去掉系统底部的重复按钮
                    val dialog = android.app.AlertDialog.Builder(requireContext())
                    dialog.setTitle("高级引擎配置")
                    ...
                    // 2. 绑定按钮点击事件
                    view.findViewById<View>(R.id.btn_debug_step).setOnClickListener {
                        android.widget.Toast.makeText(requireContext(), "单步调试功能开发中", android.widget.Toast.LENGTH_SHORT).show()
                    }
                    view.findViewById<View>(R.id.btn_debug_full).setOnClickListener {
                        android.widget.Toast.makeText(requireContext(), "调试链路功能开发中", android.widget.Toast.LENGTH_SHORT).show()
                    }

                    // 3. 点击【保存】按钮的真实逻辑
                    view.findViewById<View>(R.id.btn_save_engine).setOnClickListener {
                        ...
                        val etWsUrl = view.findViewById<android.widget.EditText>(R.id.et_ws_url)
                        val etWsSendJs = view.findViewById<android.widget.EditText>(R.id.et_ws_send_js)
                        val rgAudioParse = view.findViewById<android.widget.RadioGroup>(R.id.rg_audio_parse)
                        val etCustomParseJs = view.findViewById<android.widget.EditText>(R.id.et_custom_parse_js)
                        val etCommonJsLib = view.findViewById<android.widget.EditText>(R.id.et_common_js_lib)
                        ...
                        val wsUrl = etWsUrl.text.toString()
                        val wsSendJs = etWsSendJs.text.toString()

                        var audioParseType = 1
                        when (rgAudioParse.checkedRadioButtonId) {
                            R.id.rb_binary_mp3 -> audioParseType = 1
                            R.id.rb_pcm -> audioParseType = 2
                            R.id.rb_json_base64 -> audioParseType = 3
                            R.id.rb_raw_base64 -> audioParseType = 4
                            R.id.rb_custom_js -> audioParseType = 5
                        }
                        val customParseJs = etCustomParseJs.text.toString()
                        val commonJsLib = etCommonJsLib.text.toString()

                        // 4. 打包成完整配置 JSON
                        val customConfig = AdvancedTtsConfig(
                            name = name,
                            interval = interval,
                            useWebSocket = useWebSocket,
                            preRequestsJson = preRequestsJson,
                            wsUrlTemplate = wsUrl,
                            wsSendMessageJs = wsSendJs,
                            audioParseType = audioParseType,
                            customParseJs = customParseJs,
                            commonJsLib = commonJsLib
                        )
                        ...
                        val tts = io.legado.app.data.entities.HttpTTS().apply {
                            this.name = name
                            this.jsLib = configJson
                            this.concurrentRate = "1"
                            this.url = "ws://custom-advanced-engine"
                        }
